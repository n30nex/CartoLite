...
export function recentNeighborRoutes(
  ...
  return routes.filter((route) => (
    (route.fromId === selectedNodeID || route.toId === selectedNodeID) && isRecentNeighborRoute(route, now)
  ));
}

export function isRecentNeighborRoute(route: Pick<RouteV2, 'lastHeard'>, now: number): boolean {
