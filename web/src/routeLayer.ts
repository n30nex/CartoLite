...
const ROUTE_TEXTURE_HEIGHT = 1536;
...
export class HistoricalRouteLayer implements CustomLayerInterface {
  ...
  onAdd(map: MapLibreMap, gl: WebGLRenderingContext | WebGL2RenderingContext): void {
    ...
    this.resources = {
      ...
      maximumLineWidth: Number((gl.getParameter(gl.ALIASED_LINE_WIDTH_RANGE) as Float32Array | number[])[1] ?? 1),
      textureProgram,
      textureBuffer,
      texture,
...
function historicalRouteTexture(
  ...
  const northwest = mercator(-142, 84);
  const southeast = mercator(-48, 38);
