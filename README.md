...
CartoLite is a compact, privacy-safe live map of high-confidence MeshCore Canada RF traffic. Its calm CARTO vector geography keeps the live topology readable: recent nodes cluster cleanly, the heatmap shows where traffic is active, and historical routes stay off until requested. Route windows narrow automatically from 24 hours at local detail to 15 minutes at national scale and also constrain focused-node connections. Every eligible route in the chosen window remains in the map: nearby links form weighted geographic trunks at national and regional zooms, then resolve into individual lines as the map zooms in. Selecting a node lights its recent connected routes; hovering or tapping a focused route shows its endpoints, packet kind, packet count, and last-heard time. Every visible packet hop travels as a bright protocol-coloured trail with a directional comet, small traveling sparks, relay articulation, destination bloom, and short node wake; finished paths linger for 15 seconds even when stable routes are hidden. A compact traffic meter responds without tinting or flashing the full map. An optional 34-region MeshMapper boundary-and-label overlay stays lazy until requested and remains locked to the basemap during camera movement. The opt-in Sound control gives every live hop crossing the current view one warm musical note with local volume control and packet-specific voice shading; off-screen hops and observer-only traffic stay silent. View, layer, route-window, legend, and audio preferences remain local to that browser. CartoLite deliberately omits history, chat, PacketTV, search, phonebooks, operator tools, visitor analytics, and databases.
...
- One vanilla TypeScript page uses MapLibre GL JS for vector geography, clustered nodes, bounded heat, exact MeshMapper regions, geographically anchored route trunks, individual route lines, and selected-route glow. Canvas2D is reserved for transient packet motion.
