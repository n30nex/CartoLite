...
CartoLite is a compact, privacy-safe live map of high-confidence MeshCore Canada RF traffic. Its calm CARTO vector geography keeps the live topology readable: visitors can switch between clusters and individual nodes at every zoom, while the stronger heatmap separates activity by packet type. Optional multidirectional Topo makes relief clearer; enabling 3D also enables Topo and Buildings and aligns live trails, residue, and ground rings with the terrain and camera. Desktop 3D adds mapped buildings, a compass, pitch control and adjustable terrain height (1× by default). These are illustrations of confirmed hops, not antenna heights or radio coverage. Route windows narrow automatically from 24 hours at local detail to 15 minutes at national scale and also constrain focused-node connections. Every eligible route in the chosen window remains an individual geographic line at every zoom. Selecting a node opens a persistent inspector, lights its recent connected routes, and lists its active neighbours newest first. The local Node Finder searches only already-downloaded public labels. Every visible packet hop travels as a sharp protocol-coloured core with a short tapered glow, restrained sparks, relay handoff, destination shimmer, node wake, and 45-second sparkling residue independent of the historical Routes layer. The geographic map keeps its optional region labels static and free of traffic badges. In Netgraph, cross-region traffic makes both exact area labels answer in packet colour with OUT and IN cues; long-haul DX candidates receive a stronger bounded treatment. Live Follow holds each activity for ten seconds with a node/packet card, countdown and pause/resume. Map and Netgraph share saved Night, Daylight and Streets/Paper scenes, interface themes and six route presets with advanced width, pattern, glow, packet and trail controls. The optional 193-leaf MeshCore.ca region partition stays lazy until requested, with stronger borders, subtle region colors and a viewport legend that stays locked to the basemap during camera movement. The opt-in Sound control gives every live hop crossing the current view one note in a choice of [30 sound voices](docs/sound-voices.md), including an Android-safe gesture unlock; off-screen hops and observer-only traffic stay silent. [Netgraph](https://carto.canadaverse.org/netgraph/) presents the same live feed as a stable, full-screen topology with every connected node and active-window link, exact Canadian region grouping, nearby U.S. metro grouping, synchronized packet travel and sound, local search, and neighbour inspection. [CartoLite Labs](https://carto.canadaverse.org/labs/) interprets that same sanitized live feed as Packet Pond, Firefly Meadow, Mesh Loom, and Little Mesh Villages without adding another service or data source. On supported phones CartoLite keeps the screen awake while visible and refreshes its state and live stream whenever the page resumes. The signed [CartoLite Android app](https://canadaverse.org/cartolite/) adds an immersive shell, native keep-awake, and stronger lifecycle recovery without changing those privacy boundaries. View, layer, route-window, legend, and audio preferences remain local to that browser or app. CartoLite deliberately omits history, chat, PacketTV, phonebooks, operator tools, visitor analytics, and databases.
