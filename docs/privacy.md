...
Netgraph and CartoLite Labs use the same `/api/state` and `/api/events` public boundary as the map. Netgraph derives only a deterministic topology layout and stores only its route-window choice; labels searched in Finder remain in memory and queries are never sent or persisted. Its MeshCore.ca partition and registry are bundled content-hashed assets fetched from CartoLite's own origin; node coordinates and visitor data are never sent to MeshCore.ca. Labs derives hop count, distance, bearing, recent route reuse, bounded session rates, and temporary Village grouping in memory. Neither view connects to MQTT, persists the packet stream, logs events to the console, adds analytics, or contacts a new third party. Pond and Meadow artwork is bundled into CartoLite as inert content-hashed WebP files. Loom and Village sound characters, changing experiments, pausing, resetting, and exhibition mode are session-only choices. The synthetic demo mode uses invented fixtures and makes no live API request.
