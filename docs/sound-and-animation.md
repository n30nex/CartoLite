...
When the lazy MeshCore.ca region dataset is available, a same-region packet gives its label and border one restrained packet-coloured pulse. Cross-region traffic starts an outward pulse at the sending label and schedules an inward answer at the receiving label on the same final-hop arrival timeline used by animation and sound. A cross-region endpoint span of at least 75 km is presented as a long-haul DX candidate: its colour-faithful comet is wider and brighter, endpoint and region glow lasts longer, residue gains one bounded sparkle, and a compact `DX` marker follows the live head. This is evidence of geographic span, not proof that tropospheric ducting caused the reception.
