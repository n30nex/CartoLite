...
Netgraph keeps region activity only as a bounded browser-memory cue list and collapses bursts to the strongest active cue per area on each frame. It does not add history or a backend field. The geographic map's optional Regions layer remains static and never shows LOCAL, OUT, IN, or DX label badges.
