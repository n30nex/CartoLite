...
4. Verify `/healthz`, `/readyz`, schema v2 at `/api/state`, a live SSE packet, the desktop and phone portrait/landscape browser map, successful CARTO TileJSON/PBF/glyph requests, no raster basemap request, lazy Mapterhorn DEM/hillshade/3D loading, Android-style audio unlock, the HTTP-to-HTTPS redirect, and HSTS.
