...
Experiment modules are dynamic imports. Switching destroys the previous renderer before mounting one replacement, while a single shell owns SSE, animation frames, sound, visibility recovery, resize, wake lock, and controls. Packet Pond, Firefly Meadow, Mesh Loom, and Little Mesh Villages use Canvas2D. Pond and Meadow request their hashed artwork only with their experiment module. Villages partitions connected topology geographically, bounds the newest node set, and caches its static roads and buildings offscreen so live couriers are the only continuously repainted layer. Every renderer caps retained objects, reduces device pixel ratio on coarse pointers, stops frames while hidden, and exposes a static reduced-motion interpretation.
