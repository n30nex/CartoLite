...
Route sonification is opt-in and visitor-local. The browser creates its native Web Audio context only after a Sound-button gesture. Every public route segment crossing the exact viewport becomes one softly enveloped pentatonic note, scheduled with the same distance weights as its Canvas2D animation. Off-screen hops and observer-only events are silent, and hiding the page stops active sound. No audio, visitor telemetry, or expanded packet fields leave the browser.
